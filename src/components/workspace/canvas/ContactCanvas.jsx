...
export default function ContactCanvas() {
  ...
  const [form, setForm] = useState({
    subject: "Let's make something interesting.",
    message: "",
  });
  ...
  const submit = (e) => {
    ...
    if (!form.message.trim()) {
      setError("Ja3far says: Empty email? Write something! 😾");
      return;
    }
        ...
        <form onSubmit={submit} className="flex flex-col gap-3">
          <label className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 rounded-md border-2 border-ink bg-muted/40 px-2 py-1.5">
              <span className="grid h-5 w-5 place-items-center rounded-sm border-2 border-ink bg-pink text-[10px]">
                H
              </span>
...
<AnimatePresence>
          ...
          <div className="flex items-center justify-between">
            <span className="font-mono text-[9px] text-muted-foreground">
              esc to close · nothing will break. probably.
            </span>
            <button
              type="submit"
              data-cursor="SEND IT ✦"
              className="rounded-md border-2 border-ink bg-green px-4 py-2 font-mono text-[11px] font-bold uppercase tracking-wide text-paper chunk-sm transition hover:-translate-y-0.5 active:translate-y-0"
            >
              Send it ✦
            </button>
          </div>
        ...
        <AnimatePresence>
          {sent && (
            ...
            >
              ...
              <div>
                <div className="font-mono text-[12px] font-bold uppercase text-ink">
                  Message delivered.
                </div>
                <p className="font-mono text-[10px] text-ink/70">
                  Ja3far approves. Real replies land at {profile.email}.
                </p>
              </div>
              <button
                onClick={() => setSent(false)}
                data-cursor="AGAIN"
                className="ml-auto rounded border-2 border-ink bg-paper px-2 py-1 font-mono text-[9px] font-bold uppercase text-ink hover:bg-muted/30"
              >
                Send another
              </button>
            </motion.div>
          ...
          {[
            {
              id: "linkedin",
              label: "LinkedIn",
              note: "linkedin.com/in/hanaa-mohammed-5b9384369",
            ...
            >
              ...
              <div className="flex-1">
                <div className="font-mono text-[12px] font-bold uppercase tracking-wide text-ink">
                  {l.label}
                </div>
              ...
              <span className="font-mono text-[12px] text-ink transition group-hover:translate-x-1">
                ↗
              </span>
            </a>
          ))}
